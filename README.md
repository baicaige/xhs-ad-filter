...
插件会在小红书页面隐藏或标记疑似广告评论，并把新广告样本写入待审核池。推荐页帖子会额外使用 `postKeywords` 过滤：标题、正文或卡片文本命中帖子关键字时，整张帖子卡片会直接隐藏。
...
1. 打开 `admin.html`。
2. 填写 Worker API 和后台口令 `ADMIN_KEY`。
3. 点击 `刷新规则库`，读取 GitHub 上的规则和 `pending.json`。
4. 在 `待审核` 区域查看样本：
   - `评论广告`：点 `通过` 后，样本关键词会加入 `rules.json` 的 `keywords`。
   - `推荐页帖子`：点 `通过` 后，样本关键词会加入 `rules.json` 的 `postKeywords`。
   - 不想加入规则的样本点 `忽略`。
5. 插件刷新规则后，新规则会在所有设备生效。
...
右侧规则列表里的规则标签可以点击删除。删除会通过 Worker 写回 `rules.json`。

帖子过滤规则示例：

```json
{
  "postKeywords": ["找对象", "找对像", "脱单", "搭子群"]
}
```
